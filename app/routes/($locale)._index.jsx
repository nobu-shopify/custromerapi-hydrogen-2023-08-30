...
export let user = null; // User info from Customer Account API
export function deleteUser() { user = null;}
...
export async function loader({params, context, request}) {
  ...
  if (loggedIn) {
     ...
     *  -> CUSTOMER API TOKEN IS NOT YET COMPATIBLE WITH STOREFRONT/CART API!!
     * 
    ...
    */
    ...
    user = await customerApi(context, query, origin);
//    console.log('company', user.data.businessAccount.company);
  }
        ...
        <Suspense>
          <div>
            <h1 className="text-4xl"> 
              Welcome!! {userData.data.customer.firstName} {userData.data.customer.lastName}
            </h1>
            <p> [creationDate] {userData.data.customer.creationDate} </p>
            <p> [emailAddress] {userData.data.customer.emailAddress.emailAddress} </p>
          </div>
        </Suspense>
      )}
...
export const HOMEPAGE_FEATURED_PRODUCTS_QUERY = `#graphql
  ...
  @inContext(country: $country, language: $language) {
    products(first: 8, query: "tag:motorcycle") {
      nodes {
        ...ProductCard
      }
    }
...
async function isLoggedIn(context, request) {
  ...
  if (session.get('expires_at') < new Date().getTime()) {
    ...
    console.log('token still valid (no refresh)');
    return true;
  }
...
export async function customerApi(context, query, origin) {
  const userAgent =
  'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.93 Safari/537.36';
  const variables = {}
...
//    console.log('origin', origin);
  ...
  const fetchUrl = `https://shopify.com/${context.env.SHOPIFY_STORE_ID}/account/customer/api/unstable/graphql`;
